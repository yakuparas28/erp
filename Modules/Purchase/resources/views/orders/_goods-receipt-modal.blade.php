{{-- Çoklu satır → tek mal kabul fişi modalı. --}}
<div id="goods-receipt-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 p-4" role="dialog">
    <div class="bg-white rounded-md shadow-xl w-full max-h-[92vh] overflow-hidden" style="max-width: min(720px, calc(100vw - 32px));">
        <form method="POST" action="{{ route('app.purchase.orders.goods-receipts.create', $po) }}" class="flex flex-col h-full">
            @csrf
            <div class="flex items-center justify-between border-b border-border-color px-4 py-3">
                ...
                <button type="button" data-po-close-receipt class="text-default hover:text-gray-900 cursor-pointer">
            ...
            <div class="p-4 overflow-y-auto space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                        <label class="text-[12px] text-default block mb-1">{{ __('Receipt Date') }}</label>
                    ...
                    <div>
                        <label class="text-[12px] text-default block mb-1">{{ __('Waybill No') }}</label>
                        <input type="text" name="waybill_no" value="{{ old('waybill_no') }}"
                               class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">
                    </div>
                    <div>
                        <label class="text-[12px] text-default block mb-1">{{ __('Warehouse') }}</label>
                        <select name="receiving_location_id" required class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">
                ...
                <div>
                    <label class="text-[12px] text-default block mb-2 font-semibold">{{ __('Lines to Receive') }}</label>
                    <table class="w-full text-sm border border-border-color rounded-md">
                        <thead class="bg-light">
                            <tr class="text-[11px] uppercase text-default">
                                ...
                                <th class="text-right py-2 px-2 w-32">{{ __('Receive Qty') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            @php $idx = 0; @endphp
                            @foreach ($po->lines as $line)
                                @php $remaining = bcsub((string) $line->qty, $line->receivedQty(), 4); @endphp
            ...
            <div class="border-t border-border-color px-4 py-3 flex items-center justify-end gap-2">
                <button type="button" data-po-close-receipt class="btn-sm bg-white border border-border-color text-gray-900 hover:bg-light cursor-pointer">
                    {{ __('Cancel') }}
                </button>
                <button type="submit" class="btn-sm bg-primary text-white border border-primary hover:bg-primary/90 cursor-pointer inline-flex items-center gap-2">
                    <i class="ph ph-check"></i> {{ __('Create Goods Receipt') }}
                </button>
            </div>
        </form>
    </div>
...
<script>
(function () {
    const modal = document.getElementById('goods-receipt-modal');
    if (!modal) return;

    const open = () => { modal.classList.remove('hidden'); modal.classList.add('flex'); };
    const close = () => { modal.classList.remove('flex'); modal.classList.add('hidden'); };

    document.querySelectorAll('[data-po-open-receipt]').forEach((btn) => btn.addEventListener('click', open));
    document.querySelectorAll('[data-po-close-receipt]').forEach((btn) => btn.addEventListener('click', close));
