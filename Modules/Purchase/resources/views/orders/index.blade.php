...
<div class="flex flex-wrap items-center justify-between gap-3 mb-3 lg:mb-6">
    <h1 class="text-gray-900 text-xl font-bold mb-0">{{ __('Purchase Orders') }}</h1>
    <button type="button" data-hs-overlay="#add-purchase-order-modal" class="btn-sm bg-dark text-white border border-dark inline-flex items-center gap-2 hover:bg-primary-hover hover:border-primary-hover cursor-pointer">
        <i class="ph ph-plus"></i> {{ __('New Purchase Order') }}
    </button>
</div>
            ...
            <tbody>
                @forelse ($orders as $order)
                    <tr class="border-b border-border-color">
                        ...
                        <td class="py-2.5 px-3">
                            <a href="{{ route('app.purchase.orders.show', $order) }}" class="btn-sm bg-white border border-border-color text-gray-900 inline-flex items-center gap-2 hover:bg-light">
                                {{ __('View') }}
                            </a>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="5" class="py-8 text-center text-sm text-default">{{ __('No purchase orders yet.') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
