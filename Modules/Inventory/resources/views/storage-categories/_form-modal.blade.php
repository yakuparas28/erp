        ...
        <form method="POST" action="{{ $action }}" class="flex flex-col bg-white border shadow-sm rounded-md border-border-color w-full">
            ...
            <div class="p-4 grid grid-cols-12 gap-3">
                <div class="col-span-12">
                    ...
                    <input type="text" name="name" required value="{{ $storageCategory->name ?? '' }}" placeholder="{{ __('e.g. Ambar Rafı') }}" class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">
                </div>
                <div class="col-span-12 sm:col-span-6">
                    <label class="text-sm font-semibold text-gray-900 mb-1 block">{{ __('Max Weight') }} (kg) <span class="text-danger">*</span></label>
                    <input type="number" name="max_weight" required step="0.0001" min="0" value="{{ $storageCategory->max_weight ?? '0' }}" class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">
                </div>
                <div class="col-span-12 sm:col-span-6">
                    <label class="text-sm font-semibold text-gray-900 mb-1 block">{{ __('Allow New Product') }} <span class="text-danger">*</span></label>
                    <select name="allow_new_product" required class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">
                        @foreach (['mixed', 'same', 'empty'] as $policy)
                            <option value="{{ $policy }}" @selected(($storageCategory->allow_new_product ?? 'mixed') === $policy)>{{ __('storage-policy.'.$policy) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
