        ...
        <form method="POST" action="{{ $action }}" class="flex flex-col bg-white border shadow-sm rounded-md border-border-color w-full">
            ...
            <div class="p-4 grid grid-cols-12 gap-3">
                <div class="col-span-12 sm:col-span-4">
                    ...
                    <input type="text" name="code" maxlength="32" value="{{ $brand?->code }}" class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white font-mono focus:outline-none focus:ring-0">
                </div>
                <div class="col-span-12 sm:col-span-8">
                    <label class="text-sm font-semibold text-gray-900 mb-1 block">{{ __('Name') }} <span class="text-danger">*</span></label>
                    <input type="text" name="name" required maxlength="128" value="{{ $brand?->name }}" class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">
                </div>
                <div class="col-span-12">
                    <label class="text-sm font-semibold text-gray-900 mb-1 block">{{ __('Website') }}</label>
                    <input type="url" name="website" maxlength="255" value="{{ $brand?->website }}" placeholder="https://" class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">
                </div>
                <div class="col-span-12">
                    <label class="text-sm font-semibold text-gray-900 mb-1 block">{{ __('Description') }}</label>
                    <textarea name="description" rows="2" maxlength="2000" class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">{{ $brand?->description }}</textarea>
                </div>
                <div class="col-span-12 flex items-center gap-2">
                    <input type="hidden" name="is_active" value="0">
                    <input type="checkbox" id="is_active_{{ $id }}" name="is_active" value="1" class="size-4 rounded border-border-color" @checked(($brand?->is_active ?? true))>
                    <label for="is_active_{{ $id }}" class="text-sm text-default">{{ __('Active') }}</label>
                </div>
            </div>
