...
@section('title', $note->note_no)

@section('content')
<div class="flex flex-wrap items-center justify-between gap-3 mb-3 lg:mb-6">
    <div>
        <h1 class="text-gray-900 text-xl max-lg:text-lg font-bold mb-0">{{ __('Delivery Note') }} — {{ $note->note_no }}</h1>
        <p class="text-[12px] text-default mb-0 mt-1">
            ...
            · {{ optional($note->salesOrder?->partner)->name }}
        </p>
    </div>
    <a href="{{ route('app.sales.delivery-notes.index') }}" class="btn-sm bg-white border border-border-color text-gray-900 inline-flex items-center gap-2">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
    <div class="bg-white border border-border-color rounded-md p-4">
        ...
        <div class="text-gray-900 font-medium">{{ optional($note->delivery_date)->format('d.m.Y') }}</div>
    </div>
    <div class="bg-white border border-border-color rounded-md p-4">
        <div class="text-[11px] uppercase text-default mb-1">{{ __('Driver') }}</div>
        <div class="text-gray-900 font-medium">{{ $note->driver_name ?: '—' }}</div>
    </div>
    <div class="bg-white border border-border-color rounded-md p-4">
        <div class="text-[11px] uppercase text-default mb-1">{{ __('Vehicle') }}</div>
        <div class="text-gray-900 font-medium">{{ $note->vehicle_plate ?: '—' }}</div>
    </div>
</div>
...
<div class="bg-white border border-border-color rounded-md p-4">
    ...
    <div class="mt-4 bg-light rounded-md p-3">
        ...
        <div class="text-sm text-gray-900 whitespace-pre-wrap">{{ $note->notes }}</div>
    </div>
    @endif
</div>
@endsection
