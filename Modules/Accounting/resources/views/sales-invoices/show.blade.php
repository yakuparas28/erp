...
<div class="flex flex-wrap items-center justify-between gap-3 mb-3 lg:mb-6">
    <div>
        ...
        @if ($invoice->source)
            <a href="{{ route('app.sales.orders.show', $invoice->source_id) }}" class="text-sm text-default hover:underline ms-2">
                {{ __('Sales Order') }} #{{ $invoice->source->id }}
            </a>
        @endif
    </div>
    <a href="{{ route('app.accounting.sales-invoices.index') }}" class="btn-sm bg-white border border-border-color text-gray-900 inline-flex items-center gap-2 hover:bg-light">
...
@if ($invoice->status === 'draft')
    <div class="bg-white border border-border-color rounded-md p-4 mb-4">
        ...
        <form method="POST" action="{{ route('app.accounting.sales-invoices.lines.store', $invoice) }}" class="flex flex-wrap items-end gap-3">
            @csrf
            <div class="flex-1 min-w-40">
                ...
                <select name="product_id" required class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">
                    @foreach ($invoice->source->lines as $soLine)
                        <option value="{{ $soLine->product_id }}">{{ $soLine->product->name }} ({{ __('ordered') }}: {{ $soLine->qty }}, {{ __('delivered') }}: {{ $soLine->delivered_qty }})</option>
                    @endforeach
                </select>
            </div>
            <div class="w-28">
