...
<div class="flex flex-wrap items-center justify-between gap-3 mb-3 lg:mb-6">
    <h1 class="text-gray-900 text-xl max-lg:text-lg font-bold mb-0">{{ __('Sales Invoices') }}</h1>
    <div class="flex items-center flex-wrap gap-2">
        ...
        @if ($salesOrders->isNotEmpty())
            <button type="button" data-hs-overlay="#new-sales-invoice-modal" class="btn-sm bg-dark text-white border border-dark inline-flex items-center gap-2 hover:bg-primary-hover cursor-pointer">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-3">
        <div class="flex items-center gap-2 flex-wrap">
            <div class="relative w-64">
                ...
                <input type="text" id="si-search" class="w-full px-3 pe-8 py-2 h-7 text-[12px]! border border-border-color rounded-md bg-white focus:outline-none focus:ring-0" placeholder="{{ __('Search') }}">
            </div>
        </div>
        <div class="flex items-center flex-wrap gap-2">
                    ...
                    <div class="p-2 space-y-1">
                        @foreach (['draft', 'posted', 'paid', 'cancelled'] as $s)
                            <label class="flex items-center gap-2 text-sm cursor-pointer px-2 py-1.5 rounded-md hover:bg-light">
                                <input type="checkbox" value="{{ $s }}" data-si-status-filter class="size-4 rounded border-border-color text-primary focus:ring-0">
                                {{ __('invoice-status.'.$s) }}
                            </label>
                        @endforeach
                    </div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody>
                @forelse ($invoices as $invoice)
                    <tr class="border-b border-border-color hover:bg-light/50" data-si-row data-status="{{ $invoice->status }}" data-search="{{ strtolower($invoice->partner->name.' SI-'.str_pad((string) $invoice->id, 5, '0', STR_PAD_LEFT)) }}">
                        <td class="py-2.5 px-2 text-sm">
                            <a href="{{ route('app.accounting.sales-invoices.show', $invoice) }}" class="font-mono text-default hover:text-primary">#SI{{ str_pad((string) $invoice->id, 5, '0', STR_PAD_LEFT) }}</a>
                        </td>
                        ...
                        <td class="py-2.5 px-2">@include('accounting::sales-invoices._status-badge', ['status' => $invoice->status])</td>
                        <td class="py-2.5 px-2 text-right">
                            <div class="hs-dropdown [--placement:bottom-right] [--auto-close:inside] relative inline-flex">
                                ...
                                <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-40 bg-white border border-border-color shadow rounded-md mt-2 z-1" role="menu">
                                    <div class="p-2 space-y-1">
                                        <a href="{{ route('app.accounting.sales-invoices.show', $invoice) }}" class="flex items-center gap-2 px-2 py-1.5 rounded-md text-sm text-gray-900 hover:bg-light">
                ...
                @empty
                    <tr><td colspan="7" class="py-8 text-center text-sm text-default">{{ __('No sales invoices yet.') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
                    ...
                    <div class="col-span-12">
                        ...
                        <select name="sales_order_id" required class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">
                            @foreach ($salesOrders as $salesOrder)
                                <option value="{{ $salesOrder->id }}">#SO{{ str_pad((string) $salesOrder->id, 5, '0', STR_PAD_LEFT) }} — {{ $salesOrder->partner->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-span-12">
...
(function () {
    ...
    const rows = () => Array.from(document.querySelectorAll('[data-si-row]'));
    const activeStatuses = () => Array.from(document.querySelectorAll('[data-si-status-filter]:checked')).map(x => x.value);
    const apply = () => {
    ...
    document.querySelectorAll('[data-si-status-filter]').forEach(el => el.addEventListener('change', apply));
})();
</script>
@endsection
