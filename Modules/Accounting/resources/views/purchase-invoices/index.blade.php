...
<div class="flex flex-wrap items-center justify-between gap-3 mb-3 lg:mb-6">
    <h1 class="text-gray-900 text-xl font-bold mb-0">{{ __('Purchase Invoices') }}</h1>
    @if ($purchaseOrders->isNotEmpty())
        <button type="button" data-hs-overlay="#new-purchase-invoice-modal" class="btn-sm bg-dark text-white border border-dark inline-flex items-center gap-2 hover:bg-primary-hover hover:border-primary-hover cursor-pointer">
            <i class="ph ph-plus"></i> {{ __('New Purchase Invoice') }}
        </button>
    @endif
</div>
            ...
            <tbody>
                @forelse ($invoices as $invoice)
                    <tr class="border-b border-border-color">
                        ...
                        <td class="py-2.5 px-3">
                            <a href="{{ route('app.accounting.purchase-invoices.show', $invoice) }}" class="btn-sm bg-white border border-border-color text-gray-900 inline-flex items-center gap-2 hover:bg-light">
                                {{ __('View') }}
                            </a>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="5" class="py-8 text-center text-sm text-default">{{ __('No purchase invoices yet.') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
                    ...
                    <div class="col-span-12">
                        ...
                        <select name="purchase_order_id" required class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">
                            @foreach ($purchaseOrders as $purchaseOrder)
                                <option value="{{ $purchaseOrder->id }}">#{{ $purchaseOrder->id }} — {{ $purchaseOrder->partner->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-span-12">
