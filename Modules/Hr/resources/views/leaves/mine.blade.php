...
<div class="flex flex-wrap items-center justify-between gap-3 mb-3 lg:mb-6">
    <div>
        <h1 class="text-gray-900 text-xl font-bold mb-1">{{ __('My Leave Requests') }}</h1>
        <p class="text-sm text-default mb-0">{{ __('Current annual leave balance') }}: <span class="font-semibold text-title">{{ $employee->annual_leave_balance }} {{ __('days') }}</span></p>
    </div>
    <button type="button" data-hs-overlay="#add-leave-modal" class="btn-sm bg-dark text-white border border-dark inline-flex items-center gap-2 hover:bg-primary-hover">
        <i class="ph ph-plus"></i> {{ __('New Leave Request') }}
    </button>
</div>
        ...
        <tbody>
            @forelse ($requests as $r)
                <tr class="border-b border-border-color">
                    ...
                    <td class="py-2.5 px-3">
                        @if ($s === 'pending')
                            <form method="POST" action="{{ route('app.hr.leaves.cancel', $r) }}" onsubmit="return confirm('{{ __('Cancel this request?') }}')">
                                @csrf
                                <button type="submit" class="text-xs text-danger hover:underline">{{ __('Cancel') }}</button>
                            </form>
                        @endif
                    </td>
                </tr>
            @empty
                <tr><td colspan="6" class="py-8 text-center text-default">{{ __('No leave requests yet.') }}</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
        ...
        <div class="pointer-events-auto bg-white border border-border-color rounded-md w-full">
            <form method="POST" action="{{ route('app.hr.leaves.store') }}">
                ...
                <div class="p-4 space-y-3">
                    <div>
                        <label class="block text-sm text-gray-900 mb-1">{{ __('Type') }} <span class="text-danger">*</span></label>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-sm text-gray-900 mb-1">{{ __('Start') }} <span class="text-danger">*</span></label>
                            <input type="date" name="start_date" class="form-control" required>
                        </div>
                        <div>
                            <label class="block text-sm text-gray-900 mb-1">{{ __('End') }} <span class="text-danger">*</span></label>
                            <input type="date" name="end_date" class="form-control" required>
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm text-gray-900 mb-1">{{ __('Half day') }}</label>
                    ...
                    <div>
                        <label class="block text-sm text-gray-900 mb-1">{{ __('Reason') }}</label>
                        <textarea name="reason" rows="3" class="form-control"></textarea>
                    </div>
                    <div class="flex items-center gap-2">
                        <input type="hidden" name="travel_allowance_requested" value="0">
                        <input type="checkbox" name="travel_allowance_requested" value="1" id="tra">
                        <label for="tra" class="text-sm text-gray-900">{{ __('Travel allowance requested') }}</label>
                    </div>
                </div>
